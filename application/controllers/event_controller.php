...
class Event_controller extends CI_Controller{
	...
	public function getEvents(){
		...
		if($this->session->userdata("user_Mode") == "a"){
			$event = $this->mobile_session->getAllMobileSession();
		}else if(($this->session->userdata("user_Mode") == "sa") || ($this->session->userdata("user_Mode") == "s")){
			$employee = $this->employee->getEmployee($this->session->userdata("eId"));
			$cId = $employee->getCenter()->getCId();
			$events = $this->mobile_session->getAllMobileSessionByCid($cId);
		}else{
